...
export default function NavDock() {
  return (
    <nav
      ...
      className="fixed bottom-3 md:bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-6 md:gap-10 py-4 px-7 md:px-8 rounded-full bg-white/18 backdrop-blur-xl saturate-150 border border-white/30 shadow-[0_8px_32px_rgba(0,0,0,0.12)] z-[100]"
    >
      {NAV_ITEMS.map((item) => {
        if (item.label === "About me") {
          return <AboutMe key={item.label} label={item.label} iconSrc={item.iconSrc} />;
        }
